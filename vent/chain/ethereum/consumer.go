...
func (c *consumer) ConsumeInBatches(start, end uint64) error {
	for i := start; i < end; i += c.blockBatchSize {
		batchEnd := start + c.blockBatchSize
		if batchEnd > end {
			batchEnd = end
		}
		logs, err := ethclient.EthGetLogs(c.client, ethclient.Filter{
			BlockRange: rpcevents.AbsoluteRange(start, batchEnd),
			Addresses:  c.filter.Addresses,
			Topics:     c.filter.Topics,
		})
		if err != nil {
			err = c.handleError(start, end, err)
			if err != nil {
				return err
			}
...
func (c *consumer) handleError(start, end uint64, err error) error {
	...
	if errors.As(err, &rpcError) {
		...
		if rpcError.IsServerError() {
			...
			if c.retries <= c.maxRetries {
				...
				return c.ConsumeInBatches(start, end)
			}
		}
	}
	return err
